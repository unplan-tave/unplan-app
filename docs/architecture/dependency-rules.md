...
| Screen | `src/screens` | `components`, `domains`, `hooks`, 일반 `lib` 유틸 | route 파일에 구현 누수, generated API/model 직접 참조 |
...
| Domain logic | `src/domains` | `lib`, `constants` | `components` 참조, `api/` 밖 generated DTO 노출 |
...
`@/lib/api/endpoints`와 `@/lib/api/model` 직접 import는 기본적으로 `src/domains/<domain>/api/*`에서만 허용합니다. `src/screens`, `src/components`, `src/hooks`는 domain API wrapper, query/mutation hook, domain model/ViewModel을 통해 접근합니다.

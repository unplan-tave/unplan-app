...
컴포넌트 분리 기준을 지켜도 screen이 비대해질 수 있다. 예: `screens/schedule/create-card/card-create-screen.tsx`는 form, sheet, draft sync, toast interaction이 한 화면에 모일 수 있다.
...
| **기능** | `components/features/<screen-or-flow>` | 특정 화면/플로우 UI 조합 | ✗ (로컬 UI 상태만) |
| **화면** | `screens/<domain-or-area>/<screen-name>` (+ `hooks/`) | 라우팅·query·store·분기 | ✓ |
...
예: `create-card`, `card-list`, `card-view`, `card-search`, `queue-to-pin`, `home`.
...
여기서 기능끼리 import 금지는 서로 다른 feature 폴더 사이의 직접 참조를 뜻한다. 같은 feature 폴더 내부의 보조 컴포넌트는 상대 import로 참조할 수 있다.
...
query·store 조합과 분기는 `screens/<domain-or-area>/<screen-name>/hooks/`에 모은다. JSX는 분기 후 기능 컴포넌트에 props만 넘긴다.
...
**(a) 묶이는 상태는 훅으로.** `screens/<domain-or-area>/<screen-name>/hooks/`로 옮긴다.

```ts
// screens/schedule/create-card/hooks/use-card-sheets.ts
...
- header, form body, toast처럼 구역이 나뉘면 `components/features/<screen-or-flow>`로 분리. **props만** 받는다.
...
- **effect** — 범용은 `hooks/`, 화면 종속은 `screens/<domain-or-area>/<screen-name>/hooks/`.

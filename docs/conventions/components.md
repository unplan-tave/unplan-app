...
- `components/features`는 도메인명이 아니라 Figma 화면명 또는 사용자 flow명 기준입니다.
- Schedule 하위 UI라도 `create-card`, `card-list`, `card-view`, `card-search`, `queue-to-pin`처럼 screen/flow 기준으로 둡니다.
- 여러 schedule 화면에서 재사용되는 도메인 표현 UI만 `components/domain/schedule`에 둡니다.
- `shared`, `common`처럼 의미가 흐린 폴더를 만들지 않습니다.
